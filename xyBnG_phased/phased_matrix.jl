...
function pgocs(test, foo, bar, lmp, nrng, ngn, trait, fixed, plan, dF, l)
    ...
    for ign in 1:ngn
        ...
        Conn.xy.tovcf("$xy-sub", "$test/$bar.lmp", "$test/$bar.vcf")
        ...
        Threads.@threads for chr in 1:29 
            lock(file_lock) do
                generate_seg(chr, "$bar.vcf", "$test", l)
            end
                Splice("$test/segtemp/$chr.csv","$test/segtemp/$chr.seg")
                
        end

        seg2mat(test, "$test/segtemp")

    
    
        #g22 = G[ids, ids]
        g22 = deserialize("$test/mat.bin")
